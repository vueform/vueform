<script>
  import FileSlotGalleryPreview from './../../../../blank/components/elements/slots/FileSlotGalleryPreview'

  export default {
    name: 'FileSlotGalleryPreview',
    render: FileSlotGalleryPreview.render,
    data() {
      return {
        defaultClasses: { 
          container: 'form-file-preview-gallery',
          image: 'form-file-preview-gallery-image',
          image_link: '',
          image_static: '',
          img: 'form-file-preview-gallery-img',
          overlay: 'form-file-preview-gallery-overlay',
          upload: 'form-file-preview-gallery-upload',
          remove: 'form-file-preview-gallery-remove',
          progressBar: 'form-file-preview-gallery-progress-bar',
          progress: 'form-file-preview-gallery-progress',
          warning: 'form-file-preview-gallery-warning',
          warningIcon: 'form-file-preview-gallery-warning-icon',
          uploaded: 'form-file-preview-gallery-uploaded',
          uploadedIcon: 'form-file-preview-gallery-uploaded-icon',
          remove: 'form-file-preview-gallery-remove',
          removeIcon: 'form-file-preview-gallery-remove-icon',
        }
      }
    }
  }
</script>

<style lang="scss">
  .form-file-preview-gallery {
    font-family: inherit;
    font-size: inherit;
    line-height: inherit;
    color: inherit;
    display: flex;
    justify-content: flex-start;
    flex-direction: column;
    transition: .2s background;
    width: var(--form-gallery-size);
    height: var(--form-gallery-size);
    position: relative;

    &:hover {
      .form-file-preview-gallery-overlay {
        opacity: 1;
        visibility: visible;
      }

      .form-file-preview-gallery-remove {
        opacity: 1;
        visibility: visible;
      }
    }
  }

  .form-file-preview-gallery-overlay {
    position: absolute;
    left: 0;
    right: 0;
    top: 0;
    bottom: 0;
    background: rgba(255,255,255,0.5);
    transition: .3s;
    opacity: 0;
    visibility: hidden;
    display: flex;
    align-items: center;
    justify-content: center;
    flex-direction: row;
    padding: 0.75rem;
    border-radius: var(--form-border-radius);
  }

  .form-file-preview-gallery-upload {
    font-size: 0.75rem;
    white-space: nowrap;
    background: #FFFFFF;
    padding: 0.0625rem 0.4375rem;
    background: var(--form-primary);
    color: #FFFFFF;
    border-radius: calc(var(--form-border-radius) * 0.75);
    text-decoration: none;
    transition: .15s;
    outline: none;

    &:hover {
      opacity: 0.9;
    }

    &:focus {
      box-shadow: 0px 0px 0px var(--form-ring-width) var(--form-ring-color);
    }
  }

  .form-file-preview-gallery-image {
    width: 100%;
    height: 100%;
    font-size: 0;
    background: var(--form-gray-100);
    border-radius: var(--form-border-radius);
    outline: none;
  }

  .form-file-preview-gallery-img {
    width: 100%;
    height: 100%;
    object-fit: cover;
    border-radius: var(--form-border-radius);

    &[src=""], &[src="data:"], &:not([src]) {
      opacity: 0;
    }
  }

  .form-file-preview-gallery-progress-bar {
    border-radius: 0;
    height: 0.1875rem;
    position: absolute;
    left: 0.1875rem;
    bottom: 0.1875rem;
    z-index: 1;
    background: #FFFFFF;
    right: 0.1875rem;
  }

  .form-file-preview-gallery-progress {
    border-radius: 0;
    height: 100%;
    background: var(--form-primary);
    transition: .6s ease;
  }

  .form-file-preview-gallery-remove,
  .form-file-preview-gallery-uploaded,
  .form-file-preview-gallery-warning {
    display: flex;
    width: 1rem;
    height: 1rem;
    align-items: center;
    justify-content: center;
  }

  .form-file-preview-gallery-remove-icon,
  .form-file-preview-gallery-uploaded-icon,
  .form-file-preview-gallery-warning-icon {
    width: 100%;
    height: 100%;
    display: block;
    mask-repeat: no-repeat;
    mask-position: center center;
  }

  .form-file-preview-gallery-remove {
    position: absolute;
    right: 0.1875rem;
    top: 0.1875rem;
    opacity: 0;
    visibility: hidden;
    background-color: var(--form-gray-200);
    border-radius: 999px;
    transition: .15s;

    &:hover {
      background-color: var(--form-gray-300);
    }
  }

  .form-file-preview-gallery-remove-icon {
    mask-image: url("data:image/svg+xml,%3Csvg viewBox='0 0 320 512' fill='currentColor' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M207.6 256l107.72-107.72c6.23-6.23 6.23-16.34 0-22.58l-25.03-25.03c-6.23-6.23-16.34-6.23-22.58 0L160 208.4 52.28 100.68c-6.23-6.23-16.34-6.23-22.58 0L4.68 125.7c-6.23 6.23-6.23 16.34 0 22.58L112.4 256 4.68 363.72c-6.23 6.23-6.23 16.34 0 22.58l25.03 25.03c6.23 6.23 16.34 6.23 22.58 0L160 303.6l107.72 107.72c6.23 6.23 16.34 6.23 22.58 0l25.03-25.03c6.23-6.23 6.23-16.34 0-22.58L207.6 256z'%3E%3C/path%3E%3C/svg%3E");
    mask-size: 0.75rem 0.75rem;
    background-color: var(--form-gray-500);
  }

  .form-file-preview-gallery-uploaded {
    position: absolute;
    right: 0.1875rem;
    bottom: 0.1875rem;
    border-radius: 999px;
    background-color: var(--form-success-bg);
  }

  .form-file-preview-gallery-uploaded-icon {
    mask-image: url("data:image/svg+xml,%3Csvg viewBox='0 0 512 512' fill='currentColor' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M173.898 439.404l-166.4-166.4c-9.997-9.997-9.997-26.206 0-36.204l36.203-36.204c9.997-9.998 26.207-9.998 36.204 0L192 312.69 432.095 72.596c9.997-9.997 26.207-9.997 36.204 0l36.203 36.204c9.997 9.997 9.997 26.206 0 36.204l-294.4 294.401c-9.998 9.997-26.207 9.997-36.204-.001z'%3E%3C/path%3E%3C/svg%3E");
    mask-size: 0.625rem 0.625rem;
    background-color: var(--form-success-color);
  }

  .form-file-preview-gallery-warning {
    position: absolute;
    right: 0.1875rem;
    bottom: 0.1875rem;
    border-radius: 999px;
    background-color: var(--form-error-bg);
  }

  .form-file-preview-gallery-warning-icon {
    mask-image: url("data:image/svg+xml,%3Csvg viewBox='0 0 192 512' fill='currentColor' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M176 432c0 44.112-35.888 80-80 80s-80-35.888-80-80 35.888-80 80-80 80 35.888 80 80zM25.26 25.199l13.6 272C39.499 309.972 50.041 320 62.83 320h66.34c12.789 0 23.331-10.028 23.97-22.801l13.6-272C167.425 11.49 156.496 0 142.77 0H49.23C35.504 0 24.575 11.49 25.26 25.199z'%3E%3C/path%3E%3C/svg%3E");
    mask-size: 0.625rem 0.625rem;
    background-color: var(--form-error-color);
  }

  .is-removing {
    .form-file-preview-gallery {
      opacity: 0.6;
    }
  }

  .is-sorting, .form-gallery.is-disabled {
    .form-file-preview-gallery {
      &:hover {
        .form-file-preview-gallery-overlay {
          opacity: 0;
          visibility: hidden;
        }

        .form-file-preview-gallery-remove {
          opacity: 0;
          visibility: hidden;
        }
      }
    }
  }
</style>