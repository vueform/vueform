...
export const value = function (elementType, elementName, options) {
  ...
  describe('v-model with value', () => {
    it('should be v-model value if no form default, no element default', async () => {
      ...
      // Expect nullValues
      expect(el.value).toStrictEqual(options.value)
      expect(el2.value).toStrictEqual(options.value2)
      expect(form.vm.data).toStrictEqual({ el: options.value, el2: options.value2, })
      expect(app.vm.data).toStrictEqual({ el: options.value, el2: options.value2, })
      ...
      expect(el.value).toStrictEqual(options.value2)
      expect(el2.value).toStrictEqual(options.value2)
      expect(form.vm.data).toStrictEqual({ el: options.value2, el2: options.value2, })
      expect(app.vm.data).toStrictEqual({ el: options.value2, el2: options.value2, })
      
      // Watchers kick in
      await nextTick()

      expect(formChangeMock).toHaveBeenCalledTimes(1)
      expect(formChangeMock).toHaveBeenNthCalledWith(1, { el: options.value2, el2: options.value2, }, { el: options.value, el2: options.value2, })
      expect(elChangeMock).toHaveBeenCalledTimes(1)
      expect(elChangeMock).toHaveBeenNthCalledWith(1, options.value2, options.value)
      expect(el2ChangeMock).not.toHaveBeenCalled()
      ...
      expect(el.value).toStrictEqual(options.value)
      expect(el2.value).toStrictEqual(options.value)
      expect(form.vm.data).toStrictEqual({ el: options.value, el2: options.value, })
      expect(app.vm.data).toStrictEqual({ el: options.value, el2: options.value, })
      ...
      // Events should trigger accordingly
      expect(formChangeMock).toHaveBeenCalledTimes(2)
      expect(formChangeMock).toHaveBeenNthCalledWith(2, { el: options.value, el2: options.value, }, { el: options.value2, el2: options.value2, })
      expect(elChangeMock).toHaveBeenCalledTimes(2)
      expect(elChangeMock).toHaveBeenNthCalledWith(2, options.value, options.value2)
      expect(el2ChangeMock).toHaveBeenCalledTimes(1)
      expect(el2ChangeMock).toHaveBeenNthCalledWith(1, options.value, options.value2)
    })
  })
}